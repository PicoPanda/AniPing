...
1. Create the database
    1.1. Check if the database already exists
    1.2. If not, create the database

2. Create the tables
    2.1. anime_info table
        (all coming from JIKAN API)
    2.2. user_data table
        (user_mal_id INTEGER PRIMARY KEY, username TEXT, email UNIQUE TEXT, password TEXT, watch_list INTEGER)

Notes: 
- The database is created in the same directory as this file.
- The database as incremental unique IDs for each row of the table.
- The database is created with the following schema:
